...
int main()
{
	...
	*/
	...
	glm::mat4 modelaux6(1.0);//Inicializar matriz de Modelo 4x4
	glm::mat4 modelaux7(1.0);//Inicializar matriz de Modelo 4x4
	...
	while (!mainWindow.getShouldClose())
	{
		...
		/*
				...
				model = glm::translate(model, glm::vec3(-0.2f, 0.20f, 0.0f));
				...
				// ------------------------------ CREACION DE LA BASE DE LAS LLANTAS ---------------------------------
				...
				// ---------------------------- CREACION DE LAS RUEDAS TRASERAS ----------------------------------
				...
				glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
				color = glm::vec3(0.3f, 0.3f, 0.03f);
				glUniform3fv(uniformColor, 1, glm::value_ptr(color));
				sp.render();
				...
				// ---------------------------- CREACION DE LAS RUEDAS FRONTALES ----------------------------------
				...
				// ------------------------------------- CREACION DEL PRIMER BRAZO DE LA GRÚA ----------------------------------------
				...
				// ------------------------------ CREACION DEL SEGUNDO BRAZO DE LA GRÚA ------------------------------------------
				...
				// ---------------------------- CREACION DEL TERCER BRAZO DE LA GRÚA ---------------------------------------------
				model = glm::rotate(model, glm::radians(135.0f), glm::vec3(0.0f, 0.0f, 1.0f));
				model = glm::translate(model, glm::vec3(0.f, -2.5f, 0.0f));
				modelaux = model;
				model = glm::scale(model, glm::vec3(1.0f, 5.0f, 1.0f));
				...
				// ---------------------------- CREACION DE LA CABINA DE LA GRUA ---------------------------------------------
		...
		//------------------------------------- CREACION DE CUERPO DE ROBOT ARAÑA------------------------------------------
		model = glm::mat4(1.0);
		...
		// -------------------------------------------- PATA LATERAL IZQUIERZA --------------------------------------------------

		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -1.5f, -6.0));
		modelaux4 = model;
		...
		// -------------------------------------------- PATA LATERAL DERECHA --------------------------------------------------

		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -1.5f, -2.0));
		modelaux5 = model;
		...
		model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f));
		modelaux6 = model;
		model = glm::scale(model, glm::vec3(3.0f, 0.3f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 1.0f, 0.1f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangular
